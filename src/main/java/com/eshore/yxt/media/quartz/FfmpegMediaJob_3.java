...
@Component
public class FfmpegMediaJob_3 {

    public static final Logger logger = LoggerFactory.getLogger(FfmpegMediaJob_3.class);
    ...
    public synchronized void run(){
        if ((count.get()) > ffmpegCount) {
            logger.error("转码720定时任务运行失败。当前服务的定时转码任务数已经超过了"+ffmpegCount+"个。暂停增加线程数。");
            return;
        }
        int countV2 = count.incrementAndGet();// 自增1,返回更新值
        logger.info("转码720定时任务运行成功。当前服务的定时转码任务数递增到："+countV2+"个。");
        ...
        logger.info("转码720定时任务运行结束。当前服务的定时转码任务数递减到："+countV+"个。");
    }
